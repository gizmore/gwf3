...
class WebSocketServer implements WebSocketObserver{
	...
	public function initQueue($serverid)
	{
		$this->serverid = $serverid;
		$this->queue = msg_get_queue(2000 + $serverid);
	}
	 ...
	 */
	private function getSocketByResourceB($uid)
	{
		foreach ($this->sockets as $socket)
		{
			echo "CHECKING ".((int)$socket->getResource()).PHP_EOL;
			if ((int)$socket->getResource() == $uid)
			{
				return $socket;
			}
		}
