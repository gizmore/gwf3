...
elseif ($tVars['hits'] === 0)
{
	echo GWF_HTML::lang('no_math', GWF_HTML::display($tVars['term']));
...
else
{
	...
	foreach ($tVars['users'] as $user)
	{
		...
		echo '<td>'.GWF_HTML::anchor($href, $user->displayCountry()).'</td>';
		echo '<td>'.GWF_HTML::anchor($href, $user->display('user_name')).'</td>';
		echo '<td>'.GWF_HTML::anchor($href, $user->displayRegDate()).'</td>';
