...
struct s_file
{
  unsigned long id;		/* warchall-patch: id for each file */
  int fd;
  int bytes;
  struct stat stat;
  struct stat cstat;
...
/* get id from externel program */
unsigned long getid() {
	static char output[1024];
	FILE *pipe = popen("/usr/local/sbin/idprovider", "r");
	...
	pclose(pipe);	
	return strtoul(output, NULL, 10);
}

/* slit msg up at \n to have id at beginning of each logged line */
void logscript(char *msg, double timechange) {
	...
	ln = strtok(msg, "\n");
	while (ln != NULL) {
		syslog(sudosh_option.priority, "%lu%c%s", script.id, 
                                 sudosh_option.rlogDelim, ln);
	
		if (!bFirst)  
			snprintf(timebuff, BUFSIZ - 1, "0.0 %i\n", strlen(ln));
        else {
            bFirst = 0;
			snprintf(timebuff, BUFSIZ - 1, "%f %i\n", timechange, strlen(ln));
		}
		
		syslog(sudosh_option.priority, "%lu%c%s", timing.id,
					sudosh_option.rlogDelim, timebuff);
...
//		fprintf(stderr,"Testing for %s\n",argtest);
  ...
  /* warchall-patch: open syslog  && check for id file */
  openlog(progname, 0, sudosh_option.facility);

  
  script.bytes = 0;
  script.id = getid();	/* warchall-patch: assign ids for each file */
  timing.bytes = 0;
  timing.id = getid();
#ifdef RECORDINPUT  
  input.bytes = 0;
  ...
  if (fstat (input.fd, &input.stat) == -1)
	...
	if (sudosh_option.priority!=-1)
		...
		/* warchall-patch: log file-created stuff */
	syslog(sudosh_option.priority, "created%c%lu%c%s", sudosh_option.rlogDelim, 
				script.id, sudosh_option.rlogDelim, script.name);
	syslog(sudosh_option.priority, "created%c%lu%c%s", sudosh_option.rlogDelim, 
				timing.id, sudosh_option.rlogDelim, timing.name);
#ifdef RECORDINPUT
	syslog(sudosh_option.priority, "created%c%lu%c%s", sudosh_option.rlogDelim, 
				input.id, sudosh_option.rlogDelim, input.name);
#endif
	  ...
	  if ((n = read (pspair.mfd, iobuf, sizeof (iobuf))) > 0)
	    {
		  ...
		  /* FIXME: this could possibly cut one char at the end */
		  iobuf[n<sizeof(iobuf)?n:sizeof(iobuf)] = '\0';
		  logscript(iobuf, newtime-oldtime);
	    }
      ...
      if (FD_ISSET (0, &readfds))
	{
	  ...
	  if ((n = read (0, iobuf, BUFSIZ)) > 0)
	      ...
	      if (written == 0)
		{
		  ...
		  syslog(sudosh_option.priority, "%lu%c%s", input.id, 
				sudosh_option.rlogDelim, input.str);
		}
#endif
	    }
	}
