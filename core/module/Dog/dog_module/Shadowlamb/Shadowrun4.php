...
// 					echo "Testing ".$npc->getName()."\n";
	...
	public static function init()
	{
		...
		if ($inited === false)
		{
			...
			self::initCmds(DOG_PATH);
			Shadowcmd::init();
			self::initItems(DOG_PATH);
// 			self::initQuests(DOG_PATH);
			self::initCityBases(DOG_PATH);
			self::initSpells(DOG_PATH);
			self::initCityQuests(DOG_PATH);
			self::initCityNPCs(DOG_PATH);
			self::initCityLocations(DOG_PATH);
// 			self::initCities(DOG_PATH);
			self::initCityAfter();
			SR_Player::init();
...
// 		Dog_Timer::addTimer(array(__CLASS__, 'shadowTimerItems'), NULL, 60);
	...
	public static function initCmds($dir='') { GWF_File::filewalker($dir.'dog_module/Shadowlamb/cmd', array(__CLASS__, 'includeFile')); }
	public static function initCore($dir='') { GWF_File::filewalker($dir.'dog_module/Shadowlamb/core', array(__CLASS__, 'includeFile')); }
	public static function initItems($dir='') { GWF_File::filewalker($dir.'dog_module/Shadowlamb/item', array('SR_Item', 'includeItem')); SR_SetItems::validateSets(); }
	public static function initSpells($dir='') { GWF_File::filewalker($dir.'dog_module/Shadowlamb/spell', array('SR_Spell', 'includeSpell')); }
// 	public static function initCities($dir='') { self::initCityFiles($dir); self::initCityAfter(); }
// 	private static function initCityFiles($dir) { GWF_File::filewalker($dir.'dog_module/Shadowlamb/city', false, array(__CLASS__, 'initCity'), false); }
// 	private static function initCityNPCs($dir) { GWF_File::filewalker($dir.'dog_module/Shadowlamb/city', false, array(__CLASS__, 'initCityNPC'), false); }
// 	private static function initCityQuests($dir) { GWF_File::filewalker($dir.'dog_module/Shadowlamb/city', false, array(__CLASS__, 'initCityQuest'), false); }
// 	private static function initCityLocations($dir) { GWF_File::filewalker($dir.'dog_module/Shadowlamb/city', false, array(__CLASS__, 'initCityLocation'), false); }
	private static function initCityAfter() { foreach (self::$cities as $city) { $city instanceof SR_City; $city->onInit(); } }
// 	public static function initQuests($dir='') { GWF_File::filewalker($dir.'dog_module/Shadowlamb/quest', array('SR_Quest', 'includeQuest')); }
	...
	private static function initCityBases($dir)
	{
		...
		{
			GWF_File::filewalker($dir.'dog_module/Shadowlamb/city', false, array(__CLASS__, 'initCityBase'), false, $city);
		}
		
		# And the rest for backwards compatible
		Dog_Log::debug('Init remaining / new areas...');
		GWF_File::filewalker($dir.'dog_module/Shadowlamb/city', false, array(__CLASS__, 'initCityBase'), false, NULL);
	}
