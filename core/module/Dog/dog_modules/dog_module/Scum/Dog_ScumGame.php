...
final class Dog_ScumGame
{
	...
	public function __construct(Dog_Channel $channel)
	{
	...
	public function isInGame(Dog_User $user) { return in_array($user->getName(), $this->players, true); }
	public function join(Dog_User $user) { $this->players[] = $user->getName(); }
	...
	public function getAbortTime($abort_time) { return $this->cmdtime + $abort_time - time(); }
	/**
	 * @return Dog_Channel
	 */
	 ...
	 * @return Dog_Server
	 */
	public function getServer() { return $this->channel->getServer(); }
	...
	public function start()
	{
		...
		# History
		$this->history = new Dog_ScumHistory(array(
			'scumh_id' => 0,
			'scumh_server' => $this->getServer()->getID(),
		...
		foreach ($this->players as $i => $name)
		{
			...
			$this->getServer()->sendNOTICE($name, sprintf('Your cards: %s', implode(', ', $this->cards[$i])));
	...
	### Play ###
	############
	public function pass(Dog_User $user)
	{
		$this->cmdtime = time();
		$name = $user->getName();
		$msg = "$name passes.";
	...
	{
		Dog_ScumStats::updateScumStats($this);
