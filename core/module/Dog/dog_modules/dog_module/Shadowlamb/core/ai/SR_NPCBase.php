...
// 		}
	...
	public static function createEnemyParty()
	{
		...
		if (count($names) === 0)
		{
			Dog_Log::error('WARNING: Empty party is empty!');
		}
		...
		foreach ($names as $classname)
		{
			...
			{
				Dog_Log::error('Unknown NPC classname in createEnemyParty: '.$classname);
				return false;
			}
			$npcs[] = $npc;
		}
		...
		foreach ($npcs as $npc)
		{
			...
			{
				Dog_Log::error('Failed to spawn NPC: '.$npc->getNPCClassName());
			}
		}
