...
final class Dog_CVS_Svn_Worker extends Dog_CVS_Worker 
{
	################
	### Checkout ###
	################
	...
	public static function checkout($gdo_data)
	{
		...
		$command = "svn checkout $eurl $epath";
		
		$callbacks = array(
			null,
			array(__CLASS__, 'proc_checkout_out'),
			array(__CLASS__, 'proc_checkout_err'),
		);
		
		self::proc_rw_with_callbacks($command, $callbacks);
		
		self::tracInit();
	}
	
	public static function proc_checkout_out($message)
	{
	...
	public static function proc_checkout_err($message)
	{
		self::parseProgress($message);
		if (self::elapsed() > 5.0)
		{
