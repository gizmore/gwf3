<?php
final class LambModule_Warfare extends Lamb_Module
{
	const TIMEOUT = 1;
...
//		echo __METHOD__.PHP_EOL;
	...
	public function onProbe(Lamb_Server $server, Lamb_User $user, $from, $origin, $message)
	{
...
//		Lamb::instance()->addTimer($server, array($this, 'scanTimer'), NULL, self::TIMEOUT+rand(2,5));
	...
	public function onProbechans(Lamb_Server $server, Lamb_User $user, $from, $origin, $message)
	{
		$this->dest = $message;
	...
	### Scanner ###
	###############
	public function scanTimer(Lamb_Server $server, $args)
	{
		echo __METHOD__.PHP_EOL;
		switch ($this->status)
		{
	...
	private function onPopIP(Lamb_Server $server)
	{
	...
	private function onNextPort(Lamb_Server $server)
	{
		$this->curr_port++;
