...
//		$this->niceArray($langs, false, '-------')
		...
		$back = $this->_module->lang('th_lang').':'.PHP_EOL;
		$back .= '<select id="bb_code_lang_sel_'.$key.'">'.PHP_EOL;
		$back .= '<option value="0">'.$this->_module->lang('th_lang').'</option>'.PHP_EOL;
		foreach ($langs as $lang)
		{
			$back .= sprintf('<option value="%s">%s</option>', $lang, $lang).PHP_EOL;
		}
		$back .= '</select>'.PHP_EOL;
		$back .= $this->_module->lang('th_title').': <input type="text" id="bb_code_title_'.$key.'" size="20" value="" />'.PHP_EOL;
		$back .= '<input type="submit" value="'.$this->_module->lang('btn_code').'" onclick="return bbInsertCodeNow(\''.$key.'\');" />'.PHP_EOL;
		return $back;
	}
