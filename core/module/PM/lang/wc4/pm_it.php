...
$lang = array(
	...
	'err_ignored' => '%s vi ha messo sulla sua Lista Ignorati:',
...
// 		'Speriamo che le piacca il nostro sito e si possa divertire.'.PHP_EOL,
	...
	'wpm_title' => 'Benvenuto su '.GWF_SITENAME,
	'wpm_message' =>
		'Caro giocatore, benventuo su WeChall.'.PHP_EOL.
		PHP_EOL.
		'Lo scopo finale del sito è creare una classifica universale per siti di sfide.'.PHP_EOL.
		'Per questo dovresti [url=/linked_sites]collegare i suoi account su siti di sfide[/url] a WeChall.'.PHP_EOL.
		'Può farlo in Account -> Siti collegati.'.PHP_EOL.
		'Ci sono anche delle [url=/challs]sfide su WeChall[/url], e il tuo account è già collegato a WeChall stesso.'.PHP_EOL.
		'Se le piace il sito, non dimentichi di visitarlo frequentemente e [url=/linked_sites]aggiornare i suoi progressi[/url].'.PHP_EOL.
		'Se è alla prima armi con i siti di sfide, si senta libero di chiedere aiuto nei nostri forum, dove la aiutermo volentieri.'.PHP_EOL.
		'Se ha delle domande o dei consigli, la preghiamo di parlarcenes.'.PHP_EOL.
		PHP_EOL.PHP_EOL.
		'Ecco alcuni consigli per impostare il suo account (prema il bottone [url=/account]Account[/url] nel menù per farlo):'.PHP_EOL.
		'1) Il sito è incentrato sulla risoluzione di sfide, per cui risolva delle prove sui [url=/active_sites]siti affiliati[/url] e [url=/linked_sites]li colleghi al suo account[/url]. Se incontra dei problemi, non si faccia scrupoli a contattarci.'.PHP_EOL.
		'2) Può impostare le [url=/forum/options]Opzioni del forum[/url], [url=/pm/options]dei PM[/url] e [url=/profile_settings]del profilo[/url] se vuol essere contattato. Qualsiasi contatto (privato) è disabilitato per default.'.PHP_EOL.
		'3) [b]WeChall è predisposto per SSL[/b], ma non lo impone in alcuna parte del sito. Se desiderasse usarlo, il sito è predisposto per SSL. Se dovesse incontrare problemi, non esiti a contattarci.'.PHP_EOL.
		PHP_EOL.
		'Buon divertimento'.PHP_EOL.
		'Il team di WeChall'.PHP_EOL,
);
