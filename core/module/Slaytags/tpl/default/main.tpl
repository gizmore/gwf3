...
{foreach from=$history item=h}
		{include file="{$core}module/Slaytags/tpl/default/songbox.tpl" song=$h playing=false left=-1}
{/foreach}
	</div>
	<div>
		<h2>{$lang->lang('now_playing')}</h2>
		{include file="{$core}module/Slaytags/tpl/default/songbox.tpl" song=$now playing=true left=$left}
	</div>
</div>
