...
final class WeChall_API_Upstream extends GWF_Method
{
	public function execute()
	{
		...
		$query =
		"SELECT userhist_date, userhist_sid, userhist_type, user_name, site_name, site_classname,".
		" userhist_onsiterank, userhist_onsitescore, site_maxscore,".
		" userhist_totalscore, userhist_gain_score, regat_options, ".
...
// 		die($query);
		...
		while ($regat = $regats->fetch($result, GDO::ARRAY_O))
		{
			...
			$after = (int)$regat->getVar('userhist_totalscore');
			$gain = (int)$regat->getVar('userhist_gain_score');
			$before = $after - $gain;
			$json['result'][] = array(
				'username' => $regat->getVar('user_name'),
				'globalrank' => $regat->getVar('global_rank'),
				'type' => $regat->getVar('userhist_type'),
				'datetime' => $regat->getVar('userhist_date'),
				'sitename' => $regat->getVar('site_name'),
				'siterank' => $regat->getVar('userhist_onsiterank'),
				'sitescore' => $regat->getVar('userhist_onsitescore'),
				'siteclass' => $regat->getVar('site_classname'),
				'before' => $before,
				'after' => $after,
				'gain' => array(
					'score' => $gain,
					'percent' => $before != 0 ? round(($after/$before-1.00)*100, 2) : null,
				),
			);
			if ($json['time'] === null)
			{
