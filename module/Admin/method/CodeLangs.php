<?php
final class Admin_CodeLangs extends GWF_Method
{
...
//		$this->niceArray($langs, false, '-------')
		...
		$back .= '</select>'.PHP_EOL;
		$back .= $module->lang('th_title').': <input type="text" id="bb_code_title_'.$key.'" size="20" value="" />'.PHP_EOL;
		$back .= '<input type="submit" value="'.$module->lang('btn_code').'" onclick="return bbInsertCodeNow(\''.$key.'\');" />'.PHP_EOL;
		return $back;
	}
