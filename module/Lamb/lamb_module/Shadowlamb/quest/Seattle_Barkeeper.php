...
final class Quest_Seattle_Barkeeper extends SR_Quest
{
	...
	public function onInviteCitizen(SR_NPC $npc, SR_Player $player)
	{
	...
	public function onQuestSolve(SR_Player $player)
	{
		$xp = 12;
		$ny = 1000;
		$player->message(sprintf('The barkeeper hands you %s Nuyen and smiles: "Good job we got surely will have some more guests". You also gain %s XP.', $ny, $xp));
		$player->giveNuyen($ny);
		$player->giveXP($xp);
