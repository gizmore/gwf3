...
final class WeChall_ChallsProfile extends GWF_Method
{
	...
	public function templateChalls(Module_WeChall $module, GWF_User $user)
	{
		...
		$by = GDO::getWhitelistedByS(Common::getGet('pcby'), self::DEFAULT_BY, $whitelist);
		...
		$tVars = array(
			...
			'table_title' => $module->lang('tt_challs_for', '', $user->display('user_name')),
		);
		return $module->template('challs_profile.php', NULL, $tVars);
	}
}
?>