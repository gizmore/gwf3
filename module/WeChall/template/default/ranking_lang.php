...
echo GWF_Box::box($tLang->lang('pi_langrank', $tVars['langname']), $tLang->lang('pit_langrank', $tVars['langname']));
...
$rank = $tVars['rank'];
echo GWF_Table::displayHeaders($headers).PHP_EOL;
$solvetext = ' solved ';
$ontxt = ' on ';
foreach ($tVars['users'] as $user)
{
	...
	echo sprintf('<td><a href="%s" title="%s">%s</a></td>', GWF_WEB_ROOT.'profile/'.$user->urlencode('user_name'), $tLang->lang('a_title', $user->getVar('user_level')), $username);
