...
if ($user !== false) {
	...
	if (false !== ($regat = WC_RegAt::getRegatRow($userid, $siteid))) {
		...
		echo GWF_Box::box($tLang->lang('site_detail_uinfo', $user->displayUsername(), $regat->getOnsiteScore(), $max, $site->displayName(), round($regat->getPercent($max),2), WC_RegAt::calcExactSiteRank($user, $siteid), $site->calcScore($regat)));
	}
}
?>
...
<table>
	<thead>
		<tr>
			<th colspan="2" id="wc_site_detail_head"><?php echo $site->displayLogo(32, $tLang->lang('hover_logo', $site->displayName())).$site->displayLink(); ?></th>
		</tr>
	</thead>
	<?php 
	if (GWF_User::isStaffS() || WC_SiteAdmin::isSiteAdmin(GWF_Session::getUserID(), $siteid)) {
		echo WC_HTML::tableRowForm($tLang->lang('ft_edit_site', $site->displayName()), $site->getEditButton($tVars['module'], GWF_Session::getUser()));
	}
...
{
	echo '<table>';
	echo sprintf('<tr><th colspan="%d">%s</th></tr>', $cols, $tLang->lang('th_latest_players', $count, $tVars['latest_players_time']));
	
	echo '<tr>';
	
	$i = 0;
	foreach ($tVars['latest_players'] as $row)
	{
...
?>
		<?php if ($site->getScore() > 0) { ?>
			<div class="wc_graph fl"><img src="<?php echo $site->hrefGraphScore(); ?>" title="<?php echo $tLang->lang('it_graph_sitescore', $name); ?>" alt="<?php echo $tLang->lang('it_graph_sitescore'); ?>"></img></div>
		<?php }?>
		<?php if ($site->getUsercount() > 0) { ?>
			<div class="wc_graph fl"><img src="<?php echo $site->hrefGraphUsers(); ?>" title="<?php echo $tLang->lang('it_graph_siteusers', $name); ?>" alt="<?php echo $tLang->lang('it_graph_siteusers'); ?>"></img></div>
		<?php }?>
		<?php if ($site->getChallcount() > 0) { ?>
			<div class="wc_graph fl"><img src="<?php echo $site->hrefGraphChalls(); ?>" title="<?php echo $tLang->lang('it_graph_sitechalls', $name); ?>" alt="<?php echo $tLang->lang('it_graph_sitechalls'); ?>"></img></div>
		<?php }?>
		<?php #} ?>
	<div class="cl"></div>
<?php } ?>
...
GWF_Website::setPageTitle($tLang->lang('pt_site_detail', $dtitle));
GWF_Website::setMetaTags($tLang->lang('mt_site_detail', $dtitle));
GWF_Website::setMetaDescr($tLang->lang('md_site_detail', $dtitle));
?>