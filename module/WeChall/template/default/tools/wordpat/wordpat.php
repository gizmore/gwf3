...
$patterns_href = GWF_WEB_ROOT.'module/WeChall/template/default/tools/wordpat';
echo GWF_Box::box($tVars['lang']->lang('page_info', $external_href, $wordlist_href, $patterns_href), $tVars['lang']->lang('page_title'));
...
function wordpatMatch($pattern) {
	...
	$filename = GWF_SERVER_PATH.'/module/WeChall/template/default/tools/wordpat/patterns/'.$iso.'/'.$len.'.wl';
//	$filename = "tools/Wordpat/patterns/$country/$len.wl";
	if (!file_exists($filename)) {
		echo GWF_HTML::err('ERR_FILE_NOT_FOUND', GWF_HTML::display($filename));
		return false;
	}
...
<div>
	<form method="post" action="<?php GWF_WEB_ROOT.'tools/wordpat'; ?>">
		<div>
			...
			<input type="submit" name="wordpat" value="<?php echo $tVars['lang']->lang('btn_wordpat'); ?>" />
		</div>
	</form>
</div>
