...
$lang = array(
	...
	'info' => 
		...
		'[url=%2$s]Common::getPost[/url] gibt nur einen string aus den [url=%3$s]$_POST Variablen[/url] zurück, und wendet [url=%4$s]stripslashes()[/url] darauf an, falls [url=%5$s]magic_quotes_gpc()[/url] aktiviert ist.'.PHP_EOL.
		'Du kannst [url=%2$s]Common::getPost[/url] ignorieren, und es durch $_POST[\'input\'] ersetzen, und davon ausgehen, dass [url=%5$s]magic_quotes_gpc()[/url] deaktiviert ist.'.PHP_EOL.
		PHP_EOL.
		'Unter dem Eingabekasten findest du den Ausgabekasten, um deine Angriffe zu testen.'.PHP_EOL.
		'Du wirst eh versagen, da ich [url=%6$s]htmlspecialchars()[/url] verwendet habe, um XSS Angriffen vorzubeugen.'.PHP_EOL.
		PHP_EOL.
