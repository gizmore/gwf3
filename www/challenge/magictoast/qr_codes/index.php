...
if (false === ($chall = WC_Challenge::getByTitle(GWF_PAGE_TITLE))) {
	$chall = WC_Challenge::dummyChallenge(GWF_PAGE_TITLE, 1, 'challenge/magictoast/qr_codes/index.php');
}

$chall->showHeader();
$sid = GWF_Session::getSessSID();
$SOLUTION = require_once 'challenge/magictoast/qr_codes/secret.php';
...
if ($do_highlight) {
	$code = GWF_Message::display(' [PHP title=collectedSamples.json]' . file_get_contents('challenge/magictoast/qr_codes/qr_codes.include') . '[/code]', true, false, false, array());
...
function getQrSolution(WC_Challenge $chall)
{
	$qr_details = getQrDetails($chall);
	return (string) $qr_details[1] . "" . strrev((string) $qr_details[2]) . "0";
}
?>