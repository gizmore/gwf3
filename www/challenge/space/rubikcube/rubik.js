...
*
*/
/*global YUI,ajaxUpdate*/

var setCube, getCube, resetCube, compareCube;
...
YUI.add('rubik', function (Y) {
	...
	*/
	...
	compareCube = function() {
		var x = Y.one('noscript#cubestring').getContent();
		var y = getCube();
		if (x !== y) {
			console.log(y === x, 'backend=', x, 'frontend=', y);
			console.log('Warning: either your internet connection is very slow or the backend state differs!');
		}
		return y === x;
	};
	...
	Rubik.prototype = {
		...
		*/
		...
		_undoMove: function () {
			...
			if (movement) { this._doMovement(movement, true); }
			return movement;
		},
		_redoMove: function () {
			if (this._moving) {
		...
		*/
		_onEndCube:function (evt) {
			...
			var mHorizontal = Math.abs(this._deltaX) > Math.abs(this._deltaY);
			var parts = this._tempCubie.get('className').split(' ');
			this._expectingTransition = true;
		...
		_doMovement:function (m,fromQueue) {
			...
			//Y.one('noscript#cube').setContent('');
			var moveNotation = (m.face === 'C' ? m.slice : m.face);
			var rotateClockwise = m.rotate === 'right';
			if (moveNotation === 'R' || moveNotation === 'U' || moveNotation === 'B') {
				rotateClockwise = !rotateClockwise;
			}
			moveNotation += (rotateClockwise ? '': "'");
			console.log(moveNotation);
			ajaxUpdate('answers', 'api.php?move=' + encodeURIComponent(moveNotation));
