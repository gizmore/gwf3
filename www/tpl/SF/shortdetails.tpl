...
{foreach array('red', 'blue', 'green', 'orange') as $dc}
	<a title="{$SF->lang('designcolor', $SF->lang($dc))}" href="{$SF->getIndex('layoutcolor')}layoutcolor={$dc}">
		<img src="{$root}img/SF/circle_{$dc}.png" alt="{$SF->lang('designcolor', $SF->lang($dc))}">
	</a>
{/foreach}
	<a href="{$SF->getIndex('details')}details=shown">
		<img src="{$root}img/{$iconset}/add.png" alt="[+]" title="Show Details">
	</a>
</p>
